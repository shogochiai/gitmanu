 ...
 * ファイルアップロード制限ミドルウェア
 */
export const uploadLimiter = (maxSize = 100 * 1024 * 1024) => {
    return async (c, next) => {
...
export function setCookie(c, name, value, options = {}) {
    const { maxAge = 86400, // 24時間
    httpOnly = true, secure = c.req.url.startsWith('https://'), sameSite = 'Lax', path = '/', domain } = options;
 ...
 * Cookieを削除するヘルパー関数
 */
export function deleteCookie(c, name, path = '/') {
